...
class Java:
    ...
    def _java(self, class_file):
        p = subprocess.Popen(
            ['isolate', '--processes=32', '--meta=meta', '--time=1', '--stdin=in', '--run', self.bin + 'java', class_file[:-6]],
...
class Python:
    ...
    def _python(self, python_file):
        p = subprocess.Popen(
            ['isolate', '--meta=meta', '--time=1', '--stdin=in', '--run', self.bin + 'python3', python_file],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = p.communicate()
        return output.decode(), error.decode()
...
class JavaScript:
    ...
    def _node(self, js_file):
        p = subprocess.Popen(
            ['isolate', '--processes=32', '--meta=meta', '--time=1', '--stdin=in', '--run', self.bin + 'node', js_file],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = p.communicate()
        return output.decode(), error.decode()
    ...
    def compile(self, source_code):
        testutil.write_file('main.js', source_code)
